...
# danger 폴더가 settings.py에 없다면 추가해 두세요.
try:
    from config.settings import DANGER_DIR # type: ignore
except ImportError:
...
class DatasetRecorder:
    ...
    def _create_directories(self) -> None:
        ...
        self.danger_dir.mkdir(parents=True, exist_ok=True)
    ...
    def _select_directory(self, label: str) -> Path:
        """Label에 따라 저장 폴더 선택"""
    ...
    def save(
        ...
        directory = self._select_directory(label)

        filename = f"frame_{frame_id:06d}.npy"
